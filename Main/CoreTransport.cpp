...
void CoreTransport::initPool()
{
    ...
    DEBUG_LOGLN(F("INIT CLIENTS POOL..."));
  #endif
...
volatile bool ESPInUpdate = false;
//--------------------------------------------------------------------------------------------------------------------------------------
...
{ 
  ...
  if(checkIPD(receiveBuffer))
  {
	...
	#ifdef WIFI_DEBUG
      DEBUG_LOG(F("[ESP] REMAINING BYTES: "));
   ...
   Timer1.resume();
	...
	#ifdef WIFI_DEBUG
      DEBUG_LOGLN(F("[ESP] REMAINING BYTES READED."));
	#endif		
...
volatile bool SIM800InUpdate = false;
//--------------------------------------------------------------------------------------------------------------------------------------
...
void CoreSIM800Transport::update()
{
  ...
  // +RECEIVE BUG FIX /////////////////////////////////////////////
      ...
      DEBUG_LOGLN(String(remainingDataLength));
        ...
        #ifdef SIM800_INCOMING_DATA_TIMEOUT
        ...
        while(remainingDataLength > 0)
        {
          ...
          #ifdef SIM800_INCOMING_DATA_TIMEOUT
          if(millis() - incomingDataTmr > SIM800_INCOMING_DATA_TIMEOUT)
          {
            DEBUG_LOGLN(F("SIM800: READING TIMEOUT #0!!!"));
            rebootModem();
            return;
          }
          #endif // SIM800_INCOMING_DATA_TIMEOUT
			
    			// NEW CODE //////////////////////////////////////////
    			#if defined(USE_EXTERNAL_WATCHDOG) && defined(SIM800_CALM_WATCHDOG_DURING_IPD_READ)
    			updateExternalWatchdog();
    			#endif
