...
<p>
 <ul>
  ...
  <li>Использовать разные шлюзы для связи с сетью: GSM/GPRS (SIM800), Wi-Fi (ESP8266);</li>
  ...
  <li>Работать с TFT-дисплеем - 7'' TFT под Arduino Due;</li>
  <li>Работать с микроконтролером Arduino Due и любыми платами на его основе;</li>
...
<li>В папке <b>EXTENDED_VERSION\Firmware\Arduino DUE\Main</b> - исходники прошивки;</li>
<li>В папке <b>ESP_AT</b> - прошивка для ESP8266;</li>
<li>В папке <b>EXTENDED_VERSION\SOFT</b> - текущая версия конфигуратора, коннектится к контроллеру по COM-порту;</li>
<li>В папке <b>EXTENDED_VERSION\Firmware\Arduino DUE\Libraries</b> - сторонние библиотеки, искользуемые в проекте (их количество неуклонно приближается к нулю, но пока - как есть);</li>
...
<summary><b>Кликните, чтобы посмотреть...</b><br/><br/></summary>
  ...
  * К МК у меня подключен как минимум один универсальный модуль, для тестирования работы по 1-Wire и RS-485.
</details>

Настройки прошивки по умолчанию могут меняться с обновлениями, поэтому будьте внимательны, и проверяйте, пожалуйста,
соответствие настроек прошивки вашим чаяниям. Обращаю внимание, что прошивку можно настроить по разному: можно менять кол-во датчиков, номера пинов для них, можно управлять пинами реле фрамуг 
...
<ul>
<li><b>Установить библиотеки (в папке EXTENDED_VERSION\Firmware\Arduino DUE\Libraries архива) в среду Arduino IDE!</b></li>
<li><b>ВНИМАТЕЛЬНО ПРОЧИТАТЬ ВСЕ ИНСТРУКЦИИ, РАСПОЛОЖЕННЫЕ СВЕРХУ В ФАЙЛЕ MAIN.INO!!!</b></li>
<li><b>Все настройки прошивки перед компиляцией - в файлах Configuration_*.h!</b></li>
</ul>
<p>
Распаковать архив в папку на диске, установить библиотеки из папки <b>EXTENDED_VERSION\Firmware\Arduino DUE\Libraries</b> архива в среду Arduino IDE. Затем открыть в Arduino IDE файл <b>Main.ino</b>, настроить директивы условной компиляции (файлы `Configuration_*.h`), скомпилировать прошивку и закачать в контроллер. Подсоединить железки в зависимости от выбранных настроек прошивки. <b>Если используется модуль Wi-Fi или модуль логгирования информации - подключить модуль SD-карты и закачать файлы из папки SD в корень SD-карточки!</b>
<p>
Открыть конфигуратор и подключиться к COM-порту, на котором висит контроллер, и начать общение с ним. Принципиальная схема подключения некоторого железного добра указана ниже (<b>обратите внимание</b>, что схема - это не конкретное руководство, а лишь <b>ПРИМЕР</b> подключения, т.к. прошивка очень гибко настраивается):<br/>
<img src="plan.png"/>
